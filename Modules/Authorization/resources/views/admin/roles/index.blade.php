...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Roles</h1>
        </div>
        <a href="{{ route('admin.authorization.roles.create') }}" class="px-4 py-2 bg-[#1b1b18] dark:bg-[#eeeeec] text-white dark:text-[#1C1C1A] rounded-sm hover:bg-black dark:hover:bg-white transition-colors">
            + New Role
        </a>
    </div>
        ...
        <table class="min-w-full divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
            <thead class="bg-gray-50 dark:bg-[#1C1C1A]">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Users</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Authorizations</th>
            ...
            <tbody class="divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                @forelse($roles as $role)
                    <tr>
                        ...
                        <td class="px-6 py-4">{{ $role->role_authorizations_count }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.authorization.roles.edit', $role) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300">Edit</a>
                            <form action="{{ route('admin.authorization.roles.destroy', $role) }}" method="POST" class="inline-block ml-4">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-[#706f6c] dark:text-[#A1A09A]">No roles found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
