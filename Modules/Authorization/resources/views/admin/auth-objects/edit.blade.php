...
@section('title', 'Edit Auth Object')
...
<div class="max-w-4xl">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Edit Authorization Object</h1>
    </div>

    <form action="{{ route('admin.authorization.auth-objects.update', $authObject) }}" method="POST" class="bg-white dark:bg-[#161615] border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg p-6 shadow-sm space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="code" class="block text-sm font-medium mb-1">Code *</label>
            <input type="text" name="code" id="code" required value="{{ old('code', $authObject->code) }}" class="w-full px-3 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm bg-white dark:bg-[#161615] focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white">
        </div>

        <div>
            <label for="description" class="block text-sm font-medium mb-1">Description</label>
            <textarea name="description" id="description" rows="3" class="w-full px-3 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm bg-white dark:bg-[#161615] focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white">{{ old('description', $authObject->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <div id="fields-container" class="space-y-4">
                @foreach($authObject->fields as $index => $field)
                    <div class="border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm p-4 space-y-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="fields[{{ $index }}][code]" value="{{ $field->code }}" required class="w-full px-3 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm bg-white dark:bg-[#161615]">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Label</label>
                            <input type="text" name="fields[{{ $index }}][label]" value="{{ $field->label }}" class="w-full px-3 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm bg-white dark:bg-[#161615]">
                        </div>
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="fields[{{ $index }}][is_org_level]" value="1" {{ $field->is_org_level ? 'checked' : '' }} class="rounded">
                            <label class="text-sm">Organization Level</label>
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Sort</label>
                            <input type="number" name="fields[{{ $index }}][sort]" value="{{ $field->sort }}" min="0" class="w-full px-3 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm bg-white dark:bg-[#161615]">
                        </div>
                    </div>
                @endforeach
            </div>
...
<script>
let fieldIndex = {{ $authObject->fields->count() }};
function addField() {
