...
<div class="max-w-2xl">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Create Company</h1>
    </div>
    ...
    <div class="bg-white dark:bg-[#161615] border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg p-6 shadow-sm">
        <form action="{{ route('company.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name') }}" class="w-full px-3 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm bg-white dark:bg-[#161615] text-[#1b1b18] dark:text-[#EDEDEC] focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white">
                @error('name')
